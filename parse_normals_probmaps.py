...
def plotSlice_outlines(probmap, breastm, zslice, ax, ploi, bg_img=None, plotpts=True, plotbmpts=True, plotdelau=True):
...
def run_parseprobmaps():
    ...
    while ( line ) : 
        ...
        # Display breastm 
        ax[1,0].imshow(breastm[rands,:,:], cmap=plt.cm.gray)
        ...
        outline_breastm = find_contours(breastm[rands,:,:], 0)[0]
        ...
        # Display probmap 
        ax[1,2].imshow(probmap[rands,:,:], cmap=plt.cm.gray)
        ...
        ax[1,3].axis((0, breastm[rands,:,:].shape[1], breastm[rands,:,:].shape[0], 0))
        ax[1,3].imshow(masked_probmap[rands,:,:], cmap=plt.cm.gray)
        ...
        ax[1,4].axis((0, breastm[rands,:,:].shape[1], breastm[rands,:,:].shape[0], 0))
        ...
        for k in range(5):
            ...
            ax[2,k].imshow(mask_mriVols[k][rands,:,:], cmap=plt.cm.gray)
        ...
        #show and save                     
        fig.savefig( os.path.join( parseprobmaps_path,'{}_{}_{}_{}_parse_imgprobmaps.pdf'.format(str(lesion_id),fStudyID.zfill(4),AccessionN,sideB) ) )
        
        
        ####################
        # 4) Display non query points: go through the rest of 2D slices and extract contours from prob map
        #################### 
        ...
        for zslice in range(probmap.shape[0]):
            outlines_queryprob = find_contours(masked_probmap[zslice,:,:], 0.615)
            if(outlines_queryprob):
                outlines_pts = np.concatenate(outlines_queryprob)
                ...
                ax[zslice].imshow(masked_probmap[zslice,:,:], cmap=plt.cm.gray)
        ...
        #show and save                     
        fig.savefig( os.path.join( parseprobmaps_path,'{}_{}_{}_{}_outlines_nonquery.pdf'.format(str(lesion_id),fStudyID.zfill(4),AccessionN,sideB) ) )
        
        ####################
        # 5) Display non query triangulations: go through the rest of 2D slices and extract contours from prob map
        #################### 
        ...
        for zslice in range(probmap.shape[0]):
            ...
            if(outlines_queryprob):
                ...
                alls_queryg_delaunay.append(nonqueryg_delaunay)                
             
        
        ####################
        # save all slices collection of query graph triangulations
        ####################
        allqueryg   = gzip.open(os.path.join(output_path,'{}_{}_{}_{}_allquerygraphs_Delatriang.pklz'.format(str(lesion_id),fStudyID.zfill(4),AccessionN,sideB)), 'wb')
        pickle.dump(alls_queryg_delaunay, allqueryg, protocol=pickle.HIGHEST_PROTOCOL)
        allqueryg.close()
        
        #show and save
        fig.savefig( os.path.join( parseprobmaps_path,'{}_{}_{}_{}_delaunay_nonquery.pdf'.format(str(lesion_id),fStudyID.zfill(4),AccessionN,sideB) ))
        
        
        # explore resulting graph in interesting sites  
        print "======================================\n random selected Slice: %i \n======================================\n" % rands
        fig, ax = plt.subplots(nrows=1,ncols=3, figsize=(20, 10))
        zslice = int(rands)
        ploi = 0  
        plotSlice_outlines(masked_probmap, breastm, zslice, ax, ploi, plotpts=False, plotbmpts=False, plotdelau=False) 
        ploi = 1
        plotSlice_outlines(probmap, breastm, zslice, ax, ploi, plotpts=True, plotbmpts=False, plotdelau=False)
        ploi = 2
        plotSlice_outlines(probmap, breastm, zslice, ax, ploi, bg_img=mriVols[4], plotpts=True, plotbmpts=True, plotdelau=True)
        
        #show and save  
        plt.show(block=False)                    
        fig.savefig( os.path.join( parseprobmaps_path,'{}_{}_{}_{}_zoomin_w4thpost_{}slice_{}.pdf'.format(str(lesion_id),fStudyID.zfill(4),AccessionN,sideB,str(zslice),'normal')) )
        plt.close("all")
